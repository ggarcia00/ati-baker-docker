...
//
// --------------------------------------------------------------------------------
// $Id: pclzip.lib.php 2 2018-06-09 10:45:22Z Manuela $
// --------------------------------------------------------------------------------
...
class PclZip
{
  ...
  // Constants
    const READ_BLOCK_SIZE = 2048; // 2048
...
//                case self::CB_POST_LIST :
    ...
    protected function privAdd($p_filedescr_list, &$p_result_list, &$p_options)
    {
        ...
        if (($v_zip_temp_fd = @fopen($v_zip_temp_name, 'wb')) == 0) {
            ...
            $this->privErrorLog(
                ...
                sprintf('[%05d] Unable to open temporary file \'%s\' in binary write mode',__LINE__,$v_zip_temp_name)
            );
            return self::errorCode();
        }
  ...
  // Parameters :
  // --------------------------------------------------------------------------------
    protected function privOpenFd($p_mode='wb')
    {
        $v_result = 1;
        // Look if already open
        if ($this->zip_fd != 0) {
          // Error log
            $this->privErrorLog(self::ERR_READ_OPEN_FAIL, sprintf('[%05d] Zip file \'%s\' already open',__LINE__,$this->zipname));
            return self::errorCode();
        }
        // Open the zip file
        if (($this->zip_fd = @fopen($this->zipname, $p_mode)) == 0) {
            // Error log
            $this->privErrorLog(self::ERR_READ_OPEN_FAIL, sprintf('[%05d] Unable to open archive \'%s\' in %s mode',__LINE__,$this->zipname,$p_mode));
            return self::errorCode();
        }
        return $v_result;
    }
