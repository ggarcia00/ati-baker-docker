 ...
 * @version         $Id: class.frontend.php 1277 2010-01-28 05:18:18Z Luisehahne $
 * @filesource		$HeadURL: http://svn.websitebaker2.org/branches/2.8.x/wb/framework/class.frontend.php $
...
/*
* Função que monta o menu de acordo com o css da UEL (teste??)
*/
    function menu_uel() {
                global $wb;
           if (!isset($wb->menu_number)) {
           ...
           if (!isset($wb->menu_header)) {
                $wb->menu_header = '<ul id="nivel-0">';
           }
           if (!isset($wb->menu_footer)) {
                $wb->menu_footer = '</ul>';
           }
           if (!isset($wb->menu_default_class)) {
                $wb->menu_default_class = ' class="submenu"';
           }
           if (!isset($wb->menu_parent)) {
                $wb->menu_parent = 0;
           }
           $wb->show_menu_modificado();
           //Marcelo - 12/01/2011 - 09:40 [separador inserido manualmente]
        }


    function show_menu_modificado() {
        global $database;
        global $aux_state;
        if ($this->menu_start_level>0) {
            $key_array=array_keys($this->page_trail);
        ...
        if ($this->menu_level == 0) $aux_state='none';
            if ($this->menu_recurse==0)
                return;
            // Check if we should add menu number check to query
            if($this->menu_parent == 0) {
            ...
            if($query_menu->numRows() > 0) {
                // Print menu header
                //echo "possui submenu<br>";
                if (($this->menu_level == 1) OR ($this->aux_state == 'filho')){
                    echo '<ul>';
                } else echo '<ul id="nivel-0">';
                /*
                if ($this->level == 0) {echo "\n".$this->menu_header;}
                else echo "\n";
                */
                /*
                echo "\n".$this->parent;
                ...
                echo "\n".$this->level;
                */
                //echo "/n".$this->aux_state;
                // Loop through pages
                while($page = $query_menu->fetchRow()) {
                    //Marcelo - 12/01/2011 - 14:43 [Verifica o tipo da pagina]
                    $Get_Module = $database->get_one("SELECT module FROM ".TABLE_PREFIX."sections WHERE page_id=".$page['page_id']);
                    if(strcmp($Get_Module,"separador_menu")==0)
                        $SeparadorMenu=true;
                    else
                        $SeparadorMenu=false;
                    if(!$SeparadorMenu) {
                        //Marcelo - 14/01/2011 - 13:33 [Verifica se o item do menu possui submenu]
                        $Get_SubMenu = $database->get_one("SELECT COUNT(parent) FROM ".TABLE_PREFIX."pages WHERE parent=".$page['page_id']);
                        if($Get_SubMenu>0)
                            $PossuiSubMenu=True;
                        else
                            $PossuiSubMenu=false;
                        $aux_state = 'segundo';
                        // Check if this page should be show
                        // Create vars
                        $vars = array('[class]','[a]', '[/a]', '[menu_title]', '[page_title]');
                        // Work-out class
                        if($page['page_id'] == PAGE_ID) {
                            $class = $this->menu_current_PAGE_IDclass;
                        } else {
                            $class = $this->menu_default_class;
                        }
                        // Check if link is same as first page link, and if so change to WB URL
                        ...
                        // Create values
                        //Marcelo - 11/01/2011 16:43 [Obtem o item do menu]
                        $values = array($class,'<a href="'.$link.'" target="'.$page['target'].'" '.$class.'>', '</a>', htmlentities($page['menu_title']), htmlentities($page['page_title']));
                        // Replace vars with value and print
                        $linkMenu = str_replace($vars, $values, $this->menu_item_template);
                        if(!$PossuiSubMenu)
                            $linkMenu = str_replace("<li class=\"submenu\">","<li>",$linkMenu);
                        echo "\n".$linkMenu;
                        //echo "l:".$PossuiSubMenu;
                        //echo "SELECT module FROM ".TABLE_PREFIX."sections WHERE page_id=".$page['page_id']." AND $this->extra_where_sql<br>$Get_Module";
                        // Generate sub-menu
                        if($this->menu_collapse==false OR ($this->menu_collapse==true AND isset($this->page_trail[$page['page_id']]))) {
                            $this->menu_recurse--;
                            $this->menu_parent=$page['page_id'];
                            $this->aux_state='filho';
                            $this->show_menu_modificado();
                        }
                        echo "\n".$this->menu_item_footer;
                    } else {
                        //echo "<li ".$class.">";
                        ...
                        echo "<a>";
                        //echo "<a $class>";
                        echo "<div id='separador-titulo'>";
                        echo "<div id='separador-titulo-esq'></div>";
                        echo "<div id='".($page['level']==0 ? 'separador-titulo-centro' : 'separador-titulo-centro-nivelmaior')."'>";
                        echo "<div id='separador-conteudo-titulo'>";
                        echo $page['menu_title'];
                        echo "</div>";
                        echo "</div>";
                        echo "<div id='separador-titulo-dir'></div>";
                        echo "</div>";
                        echo "</a>";
                        //echo "<div id='menu-titulo'>";
                        //echo "<div id='menu-titulo-esq'></div>";
                        //echo "<div id='menu-titulo-centro'>";
                        //echo "<div id='menu-conteudo-titulo'>";
                        //echo $page['menu_title'];
                        //echo "<a $class>".$page['menu_title']."</a>";
                        //echo "</div>";
                        //echo "</div>";
                        //echo "<div id='menu-titulo-dir'></div>";
                        //echo "</div>";
                        echo "\n".$this->menu_item_footer;
                    }
                }
